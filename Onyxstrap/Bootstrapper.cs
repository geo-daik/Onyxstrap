...
using Onyxstrap.RobloxInterfaces;
...
namespace Onyxstrap
{
    public class Bootstrapper
    {
        ...
        public string BackgroundUpdaterMutexName => $"Onyxstrap-BackgroundUpdater-{_launchMode}";

        public string MutexNamePrefix { get; set; } = "Onyxstrap-Bootstrapper";
        ...
        private async Task<bool> CheckForUpdates()
        {
            ...
            {
                App.Logger.WriteLine(LOG_IDENT, $"More than one Onyxstrap instance running, aborting update check");
                return false;
            }
            ...
            SetStatus(Strings.Bootstrapper_Status_UpgradingOnyxstrap);

            try
            {
#if DEBUG_UPDATER
                string downloadLocation = Path.Combine(Paths.TempUpdates, "Onyxstrap.exe");
